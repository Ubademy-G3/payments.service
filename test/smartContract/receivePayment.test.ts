...
import { BigNumber } from "hardhat-deploy/node_modules/@ethersproject/bignumber/lib/bignumber";

const { loadFixture } = waffle;

const { expect } = chai;

type PaymentFunction = (contractWithSigner: UbademyPayments, amountToBeSent: BigNumber) => Promise<ContractTransaction>;
...
const makeTestsPaymentToContract = (paymentFunction: PaymentFunction, functionName: string) => {
  describe(`UbademyPayments - Send payments to contract through ${functionName}`, function () {
    describe(`GIVEN the Smart Contract was deployed`, () => {
      let basicPayments: UbademyPayments;
